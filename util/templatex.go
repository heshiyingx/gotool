...
import (
	"bytes"
	errs "github.com/heshiyingx/gotool/errext"
...
func (t *DefaultTemplate) SaveTo(data any, path string, forceUpdate bool) error {
	if pathext.FileExists(path) && !forceUpdate {
		return nil
	}
...
func (t *DefaultTemplate) Execute(data any) (*bytes.Buffer, error) {
	...
	if err != nil {
		...
		return nil, errs.Wrap(err, "template parse error:", t.text)
	}

	buf := new(bytes.Buffer)
	if err = tem.Execute(buf, data); err != nil {
		return nil, errs.Wrap(err, "template execute error:", t.text)
	}
	...
	if err != nil {
		return nil, errs.Wrap(err, "go format error:", buf.String())
	}
